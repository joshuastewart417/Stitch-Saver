...
[3866,"Mocha Brn Ult Vy Lt",250,246,240]];
    ...
    const distanceFromColor = (idx, r, g, b) => {
        let tr = dmcColorList[idx][2];
	    let tg = dmcColorList[idx][3];
	    let tb = dmcColorList[idx][4];
 ...
 const matchDMC = () => {
    ...
    for(let s = 0; s < swatches.length; s++){
        for(let w = 0; w <swatches[s].length; w++ ) {
            ...
            for (let idx = 0; idx < dmcColorList.length; idx++) {
                let candidateDist = distanceFromColor(idx, rVal, gVal, bVal);
                distanceList[idx] = [candidateDist, idx];
            }

            let yes = distanceList.sort(function(a, b){return a[0]-b[0]});

            // let dmcI = dmcColorList[distanceList[0][1]][0];
            let dmcDesc = dmcColorList[distanceList[0][1]][1];
	        let dmcR = dmcColorList[distanceList[0][1]][2];
	        let dmcG = dmcColorList[distanceList[0][1]][3];
	        let dmcB = dmcColorList[distanceList[0][1]][4];

            console.log(dmcDesc, dmcR, dmcG, dmcB)
